...
- **Tech-note index:** **Rebuild the tech-note index…** finds every document whose front matter carries both a `GUID` and `TechNote` in its `keywords` — the two the TechNote template writes — and generates `MD_Internal\TechNotes.md`, a table of TN index, title, version, subject, GUID and file. Derived from the documents each time rather than accumulated, so notes made outside MD Boss, renamed, moved or deleted all come out right. New notes are **numbered `2026.04`** — the year and a sequence that restarts each January, taken one past the highest already in use; a number two notes both claim is reported under the table.
