...
**Back and Forward keep their own history, and it is deliberately not the recents list.** `history_` records every document actually SHOWN, browsing included — after arrowing through a folder, "the one I was just looking at" is the only reading that matches the screen — while recents record what was *chosen*. Four things hold it together: `going_back_` stops the move recording itself (or Back toggles between two documents for ever), consecutive duplicates collapse so a save or a reload is not a move, the list is bounded at `kMaxHistory`, and an unreachable entry is dropped and skipped past rather than stopping Back for the rest of the session. Note the order in `on_back()`: nothing is removed until the move has actually happened, so a failed attempt leaves the history exactly as it was — and the entry erased on failure is the *target*, not the current document, which an earlier version got wrong and thereby skipped a step.

**A CURSOR, not a stack.** Back was a stack that popped the current entry, which meant the documents ahead of you were destroyed the moment you moved -- fine while Back was alone, impossible once Forward existed. `history_pos_` indexes the document on screen; Back and Forward are one `navigate_history(step)` with `step` of -1 or +1, so the two can never drift apart. Opening something new **truncates everything after the cursor**, exactly as a browser does. Three details that are easy to get wrong: a failed open removes the TARGET entry, never the current one, and shifts the cursor only when the removal was before it; nothing is moved until the open has actually succeeded; and re-showing the document already on screen is not a move, so a refresh or a save does not fill the list.

**Splitting `on_context_menu()`** into `build_context_menu()` and `bind_context_menu()` (it had passed 190 lines) leaves one hazard worth knowing: an item appended in one half and not wired in the other simply does nothing when clicked, silently. Both halves take the same `templates`/`shown`, read once by the caller, because binding an INDEX into a list rebuilt per menu would read whichever list existed when the item was clicked. A cheap check after any change there is to compare the `kId*` names appearing in each half -- they must match exactly, 15 of them at the time of writing.
...
- **The Title cell is a link to the note**, written as an ABSOLUTE `file:///` URL. Relative would not do: the index lives in `MD_Internal` under one root while the notes it lists may be under any of them, and between two drives there is no relative path at all. `file_url()` encodes spaces (a raw one ends a Markdown destination early) and the title's own `[`/`]` are escaped, since either would close the link text.
...
- **A link to another MARKDOWN document opens it in the app instead** (`markdown_path_from_file_url()` in **`app/LinkTarget.h`** + `set_on_open_document`). Note what this is not: the `file:` URL still never reaches `ShellExecute`. Only the markdown extensions qualify, so a link to an `.exe`, a `.bat` or a `.pdf` yields nothing and behaves exactly as it did before, and the frame re-reads and validates the file the same way File ▸ Open does. It is dispatched through `CallAfter` because the handler runs inside a WebView2 navigation callback and `open_path()` can raise a modal dialog. The page's own load is never mistaken for a link: the preview navigates to a staged `.html` in the temp folder, never to a `.md`.
...
**md4c will not autolink a URL that carries a port, and `linkify()` in `HtmlRenderer.cpp` exists for exactly that.** Measured, not guessed: `https://example.com/admin` links, `https://example.com:8443/admin` does not, and neither does `http://10.10.10.88:8443/x` or `http://localhost:3000/` — while a bare IP *without* a port links fine. It is not a corner case here, because `logins.md` lists internal services by host and port: of 13 bare URLs in the author's file, 5 stayed dead text after `MD_FLAG_PERMISSIVEAUTOLINKS` was turned on. Four things about the supplement:

- **It only ever sees what md4c declined.** md4c consumes the URLs it will handle before `on_text` is reached, so the two can never double-link the same run. That is why the supplement can be general (any `http`/`https` run in normal text) without having to know which shapes md4c takes.
- **`MD_TEXT_NORMAL` only, which buys the context for free.** Code spans and fences arrive as `MD_TEXT_CODE`, so a URL in one stays literal without a single check.
- **`link_depth` guards against nested anchors.** A URL inside a link's own *text* — `[see https://x:1/y](other)` — must stay text; nested `<a>` is invalid HTML and the browser unnests it somewhere you did not choose.
- **http and https only.** A linkifier that took schemes generally would be a route for `javascript:` or `file:` to reach an href straight out of document text, leaving the click handler's allow-list as the only guard rather than the second of two.

**One unexplained network event, recorded because it is the export-control one.** During this work a single run of the loopback check logged `GET /port-url-should-not-fetch` **and** `GET /favicon.ico` while a document holding that URL was open. It did not reproduce: ten further runs (six of the identical scenario, four bisecting anchor / code span / image / anchor+image) all logged nothing, and a control that bound the port with no MD Boss running logged nothing. The favicon request is the signature of a *browser* loading a document, not of the preview fetching a subresource, so the likeliest explanation is a leftover Edge tab from an earlier click test being reactivated — but that was never proved. **Re-run both halves of the loopback check whenever this code is touched, and treat a hit as real until it is explained.**

**The URL parser lives in its own wx-free unit, and that is the point.** `markdown_path_from_file_url()` sat inside `PreviewPane.cpp` until a rot check observed that the single most security-relevant function in the preview was also the only one no test could reach — `PreviewPane` needs wx and WebView2, so `mdrender_tests` cannot compile it. Lifting it into `app/LinkTarget.cpp`, beside `FileScan`/`InternalNotes`/`TechNotes`, made it testable and **immediately exposed a real bug**: it decoded each `%XX` escape into a *wide character*, so a path outside ASCII — percent-encoded as UTF-8 — came back as mojibake naming a file that does not exist. `café-note.md` could not be opened from a link. It decodes to **bytes** now and the whole path stays UTF-8, matching `PathUtf8.h`. The lesson generalises: a parser of untrusted input that cannot be unit-tested is not merely untested, it is where the bug will be.

**A bare URL is a link too, since `MD_FLAG_PERMISSIVEAUTOLINKS`.** Turned on because `logins.md` keeps its Link column as bare URLs — a table cell is a poor place to hand-type `[text](url)` — and it applies to every document, as GitHub does. Two things make it safe rather than merely convenient, and both are pinned by tests: the permissive rules match only http/https/ftp and mailto shapes, so **no `javascript:`, `file:` or `data:` URL can be minted from prose**, and the allow-list above checks the scheme again before `ShellExecute` ever sees it. Code spans stay literal. The flag changed **no** golden file — the corpus's only bare URL is inside a raw `<iframe>` attribute, which is an HTML block and out of reach of a span rule — but that was confirmed by running the corpus, not by reasoning about it. Check it again the same way before touching the flags.
...
- **A Link cell's spaces become `%20`, and only when the value is a URL** (`normalise_link()`, user ruling 2026-08-17). A raw space is not legal in a URL and it *truncates the link*: every autolink — md4c's and ours — ends at the first space, so what opens is a prefix of the address. Found in a real file whose URLs had been imported from a PDF that wrapped them, leaving a space at every wrap; the links opened the first ~20 characters of themselves. The guard is conditional because the column is sometimes used for a note instead, and `%20`-ing an English sentence is worse than leaving a URL unclickable.
